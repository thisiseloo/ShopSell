...
import { useParams } from "react-router-dom";
import { useDispatch, useSelector } from "react-redux";
import { addItem, increaseQty, decreaseQty } from "../../features/cart/cartSlice";
...
const ProductDetail = () => {
  ...
  const allProducts = [
    ...womenShoes.map((item) => ({ ...item, type: "shoe", uniqueId: `wshoe-${item.id}` })),
    ...womenBags.map((item) => ({ ...item, type: "bag", uniqueId: `wbag-${item.id}` })),
    ...menShoes.map((item) => ({ ...item, type: "shoe", uniqueId: `mshoe-${item.id}` })),
    ...menBags.map((item) => ({ ...item, type: "bag", uniqueId: `mbag-${item.id}` })),
    ...kidsShoes.map((item) => ({ ...item, type: "shoe", uniqueId: `kshoe-${item.id}` })),
    ...kidsBags.map((item) => ({ ...item, type: "bag", uniqueId: `kbag-${item.id}` })),
  ];
  ...
  const cartItem = cartItems.find(
    (item) => item.uniqueId === product.uniqueId && (item.size || "") === (selectedSize || "")
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-white rounded-lg shadow-lg p-4 md:p-6">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div>
            ...
            {product.category || (product.type === "shoe" ? "Gündəlik ayaqqabılar" : "Çiyin çantası")}
          </div>
            ...
            {cartItem ? (
              <div className="flex items-center gap-4">
                <button
                  onClick={() =>
                    dispatch(
                      decreaseQty({ uniqueId: product.uniqueId, size: selectedSize })
                    )
                  }
                  className="bg-gray-200 px-4 py-2 rounded text-lg font-bold"
                >
                ...
                <button
                  onClick={() =>
                    dispatch(
                      increaseQty({ uniqueId: product.uniqueId, size: selectedSize })
                    )
                  }
                  className="bg-gray-200 px-4 py-2 rounded text-lg font-bold"
                >
