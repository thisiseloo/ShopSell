...
//     return res
//       .status(409)
//       .json({ message: "Bu email ilə artıq qeydiyyat var" });
//   }
...
app.post("/api/signup", async (req, res) => {
  ...
  const userExists = users.find(u => u.email === email || u.name === name);
  if (userExists) {
    return res.status(409).json({ message: "Email və ya istifadəçi adı artıq mövcuddur" });
  }
...
// Login (email və ya username ilə)
app.post("/api/login", async (req, res) => {
  const { identifier, password } = req.body;

  const user = users.find(u => u.email === identifier || u.name === identifier);
  if (!user) {
    return res.status(401).json({ message: "Email/İstifadəçi adı və ya şifrə səhvdir" });
  }
...
// Protected route nümunəsi
app.get("/api/protected", (req, res) => {
  const authHeader = req.headers.authorization;
  if (!authHeader) return res.status(401).json({ message: "Token yoxdur" });

  const token = authHeader.split(" ")[1];
  try {
    const decoded = jwt.verify(token, JWT_SECRET);
    res.json({ message: `Xoş gəlmisiniz, user ID: ${decoded.id}` });
  } catch {
